...
$.fn.eventCalendar = function (options) {
	...
	this.each(function () {
		...
		flags.wrap
			...
			.append(
				"<div class='eventsCalendar-list-wrap'><p class='eventsCalendar-subtitle'></p><span class='eventsCalendar-loading'>loading...</span><div class='eventsCalendar-list-content'><ul class='eventsCalendar-list'></ul></div></div>"
			);
	...
	function dateSlider(show, year, month) {
		var $eventsCalendarSlider = $("<div class='eventsCalendar-slider'></div>"),
			$eventsCalendarMonthWrap = $(
				"<div class='eventsCalendar-monthWrap'></div>"
			),
			$eventsCalendarTitle = $(
				"<div class='eventsCalendar-currentTitle'><a href='#' class='monthTitle'></a></div>"
			),
			$eventsCalendarArrows = $(
				"<a href='#' class='arrow prev'><span>" +
					eventsOpts.txt_prev +
					"</span></a><a href='#' class='arrow next'><span>" +
					eventsOpts.txt_next +
					"</span></a>"
			);
		...
		if (!flags.wrap.find(".eventsCalendar-slider").size()) {
			...
			flags.wrap
				.find(".eventsCalendar-slider")
				.append($eventsCalendarMonthWrap);
		}

		flags.wrap
			.find(".eventsCalendar-monthWrap.currentMonth")
		...
		if (eventsOpts.showDayAsWeeks) {
			...
			if (eventsOpts.showDayNameInCalendar) {
				...
				for (; i < 7; i++) {
					daysList.push(
						'<li class="eventsCalendar-day-header">' +
	...
	function getEventsData(data, limit, year, month, day, direction) {
		...
		subtitle = flags.wrap.find(
			".eventsCalendar-list-wrap .eventsCalendar-subtitle"
		);
		if (!direction) {
			// first load
			...
			if (day != "") {
				subtitle.html(
					eventsOpts.txt_SpecificEvents_prev +
						eventsOpts.monthNames[month] +
						" " +
						num_abbrev_str(day) +
						" " +
						eventsOpts.txt_SpecificEvents_after
				);
			} else {
				subtitle.html(
					eventsOpts.txt_SpecificEvents_prev +
		...
		flags.wrap.find(".eventsCalendar-list").animate(
			...
			function () {
				...
				if (data.length) {
					...
					$.each(data, function (key, event) {
						...
						if (
							...
							flags.wrap
								.find(
									".currentMonth .eventsCalendar-daysList #dayList_" +
										parseInt(eventDay)
								)
				...
				if (!events.length) {
					events.push(
						'<li class="eventsCalendar-noEvents"><p>' +
							eventsOpts.txt_noEvents +
							"</p></li>"
					);
				}
	...
	function showError(msg) {
		flags.wrap
			.find(".eventsCalendar-list-wrap")
			.html(
				"<span class='eventsCalendar-loading error'>" +
					msg +
					" " +
					eventsOpts.eventsjson +
					"</span>"
